...
class ConsoleReporterSpec extends FlatSpec with Matchers with AssertionsForJUnit {
  ...
  val prM2 = ProcessedMessageMock(Tick(TickSubscription(1, Process(123), 1 second), 2), Energy.fromPower(2))
  val prM3 = ProcessedMessageMock(Tick(TickSubscription(1, Process(123), 1 second), 3), Energy.fromPower(3))
