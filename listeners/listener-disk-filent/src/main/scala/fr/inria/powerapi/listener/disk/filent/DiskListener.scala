 ...
 */
package fr.inria.powerapi.listener.disk.filent
...
trait Configuration extends fr.inria.powerapi.core.Configuration {
  /**
   * The output filent path.
   */
  lazy val filePath = load(_.getString("powerapi.listener.disk-console.out-prefix"))(Path.createTempFile(prefix = "powerapi.listener-disk-filent", deleteOnExit = false).path)

  /**
   * If result has to be append to an existing filent, or simply overrided.
   */
...
/**
 * Disk listener displaying received DiskFormulaMessage into a filent following properties
 * contained into a configuration filent.
 *
 * @author abourdon
 * Contributor: Adel Noureddine
 */
class DiskListener extends Listener with Configuration {
  lazy val output = {
    if (log.isInfoEnabled) log.info("using " + filePath + " as output filent")
