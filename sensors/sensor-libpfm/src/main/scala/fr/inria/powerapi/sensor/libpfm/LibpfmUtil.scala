...
import org.bridj.Pointer.{ allocateLongs, pointerTo, pointerToCString }
...
object LibpfmUtil {
     ...
     * @param fd: file descriptor.
     */
    def readCounter(fd: Int): Long = {
       /* values[0] = raw count
        * values[1] = TIME_ENABLED
        * values[2] = TIME_RUNNING
        */
        val counts = allocateLongs(3)
        // 8 bytes * 3 longs
        val read = CUtils.read(fd, counts, 8 * 3)
        val values = if(read > -1) {
            counts.getLongs()
        }
        else Array(0L, 0L, 0L)

        if(values(2) == 0 && values(1) == 0 && values(2) > 0) {
