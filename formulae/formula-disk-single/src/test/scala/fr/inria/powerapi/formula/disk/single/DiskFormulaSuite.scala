...
class DiskFormulaSuite extends JUnitSuite with Matchers with AssertionsForJUnit {
  ...
  def testRefreshCache() {
    val old = DiskSensorMessage(rw = Map("n/a" -> (123: Long, 456: Long)), Tick(TickSubscription(1, Process(123), 500.milliseconds)))
  ...
  def testPower() {
    ...
    val old = DiskSensorMessage(rw = Map("n/a" -> (100: Long, 200: Long)), Tick(TickSubscription(1, Process(123), duration)))
    val now = DiskSensorMessage(rw = Map("n/a" -> (500: Long, 400: Long)), Tick(TickSubscription(1, Process(123), duration)))
  ...
  def testCompute() {
    val duration = 500.milliseconds
    val tick = Tick(TickSubscription(1, Process(123), duration))
