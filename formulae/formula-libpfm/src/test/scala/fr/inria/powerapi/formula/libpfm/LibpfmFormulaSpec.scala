...
class LibpfmFormulaSpec extends FlatSpec with Matchers with AssertionsForJUnit {
  ...
  "A LibpfmFormula" should "be configured with the values written in a configuration file" in {
    val hashMap = new scala.collection.mutable.HashMap[Long, scala.collection.mutable.ArrayBuffer[Double]]()
    hashMap += (1200000L -> scala.collection.mutable.ArrayBuffer(3.975152144472587E-10,3.6663704141026026E-10,32.96899190728478))
    hashMap += (1300000L -> scala.collection.mutable.ArrayBuffer(4.1257289323769395E-10,3.6174234365649696E-10,33.01644964171548))

    libpfmFormula.underlyingActor.formulae.keys should contain(1200000L)
    libpfmFormula.underlyingActor.formulae(1200000L) should equal(hashMap(1200000L))
    libpfmFormula.underlyingActor.formulae.keys should contain(1300000L)
    libpfmFormula.underlyingActor.formulae(1300000L) should equal(hashMap(1300000L))
  ...
  "A LibpfmFormula" should "be able to listen LibpfmSensorMessage and compute the CPU consumption" in {
    ...
    val powerM1 = formula(0) * 3591479118L + formula(1) * 6768599498L
    val powerM2 = formula(0) * 1197309162L + formula(1) * 3410238673L
    ...
    val msg1T1M2 = LibpfmSensorMessage(counter = Counter(1197309162L), event = Event("cycles"), tick = Tick(2, TickSubscription(Process(123), 1.second), 1))
    val msg2T1M2 = LibpfmSensorMessage(counter = Counter(3410238673L), event = Event("instructions"), tick = Tick(2, TickSubscription(Process(123), 1.second), 1))
    ...
    listener.underlyingActor.monitoringBuffers(2)(1).energy should equal(Energy.fromPower(0.0))
  }
}